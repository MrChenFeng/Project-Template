...
# convert normalized control points to image size grid
...
# convert image size grid to normalized control points
...
class TPS(object):
    ...
    def __init__(self, H, W, warpsd_all=0.001, warpsd_subset=0.0, transsd=0.02,
    ...
    def __call__(self, image, keypts=None, crop=0):
        kp1 = 0
        ...
        a = 1
        weight = random_tps_weights(self.nctrlpts, a * self.warpsd_all, a * self.warpsd_subset, a * self.transsd,
                                    a * self.scalesd, a * self.rotsd)

        grid = torch.matmul(self.F, weight).reshape(1, self.H, self.W, 2)

        if keypts is not None:
            grid_unnormalized = grid_unnormalize(grid, self.H, self.W)
            kp1 = self.warp_keypoints(keypts, grid_unnormalized)
        ...
        # Reverse the order because due to inverse warping the "flow" is in direction im2->image
        # and we want to be consistent with optical flow from videos
        return {'image': image, 'warped_image': warped_image, 'weight': weight, 'grid': grid}
...
if __name__ == '__main__':
    ...
    #
    # grid = torch.randn(1,6,6,2)
    # output = torch.nn.functional.grid_sample(input, grid)

    from datasets import LSP
    from utils.simple_transformers import ImageToTensor, Resize
    from torchvision.transforms import Compose
    from torch.utils.data import DataLoader

    dataset = LSP(root_path='/home/chen/Video_Disentanled_Representation/Datasets/LeedsSport',
                  transform=Compose([Resize([128, 128]), ImageToTensor(), TPS(128, 128)]))
    data_loader = DataLoader(dataset, batch_size=10, drop_last=True, shuffle=True)
    # t = TPS(128, 128, warpsd_all=0.01)
    # im2 = t(im1)
    ...
    display(sample['image'][0].permute(1, 2, 0))
    display(sample['warped_image'][0].permute(1, 2, 0))

    # plt.imshow(im2[3][0][:, :, 1].cpu(), cmap='gray')
    # plt.show()
